...
class CreateSurvey201Response {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is CreateSurvey201Response &&
     other.data == data;
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CreateSurvey201Response[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CreateSurvey201Response[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<CreateSurvey201Response>? listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<CreateSurvey201Response>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CreateSurvey201Response>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CreateSurvey201Response.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
