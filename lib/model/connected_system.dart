...
class ConnectedSystem {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ConnectedSystem &&
     other.symbol == symbol &&
  ...
  @override
  String toString() => 'ConnectedSystem[symbol=$symbol, sectorSymbol=$sectorSymbol, type=$type, factionSymbol=$factionSymbol, x=$x, y=$y, distance=$distance]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ConnectedSystem[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ConnectedSystem[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ConnectedSystem>? listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<ConnectedSystem>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ConnectedSystem>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ConnectedSystem.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
