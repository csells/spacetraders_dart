...
class GetSystemWaypoints200Response {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is GetSystemWaypoints200Response &&
     other.data == data &&
     other.meta == meta;
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "GetSystemWaypoints200Response[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "GetSystemWaypoints200Response[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<GetSystemWaypoints200Response>? listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<GetSystemWaypoints200Response>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<GetSystemWaypoints200Response>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = GetSystemWaypoints200Response.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
